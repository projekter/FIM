...
abstract class Autoboxing {
    ...
    * @param string $name
    * @param array $arguments You may pass optional arguments as an associative
    *    array to the function. The keys need to match the parameter names.
    * @return mixed
    */
   public static function callMethod($class, $name, array $arguments = []) {
      $parentClass = func_get_args();
      if(isset($parentClass[3]))
         $reflection = new ReflectionClass($parentClass[3]);
      else{
    ...
    */
   public static function callFunction(callable $function, array $arguments = []) {
   ...
   private static function setupFileCache(ReflectionFile $file) {
      ...
      if(in_array(DIRECTORY_SEPARATOR === '/' ? $cacheFile : str_replace('/',
                  DIRECTORY_SEPARATOR, $cacheFile), get_included_files(), true))
...
abstract class {$class->getShortName()} {
   ...
   public static function __callStatic(\$name, \$arguments) {";
         ...
         if($parentClass === false || (dirname($fn = $parentClass->getFileName()) === __DIR__ && $rules =
            ...
            $cacheContent .= "
      # Sadly, we cannot trigger a fatal error, which would be the exact equivalent to this situation
      throw new \\BadMethodCallException('Call to undefined method ' . \\Autoboxing::\$currentlyCalling . \"::\$name\");
   }";
         elseif(isset($rules)) {
            $cacheContent .= "
      if(\$name !== 'checkExistence' && \$name !== 'checkReading' && \$name !== 'checkListing')
         ...
         return null;
            ...
            $cacheContent .= "
      return \\Autoboxing::callMethod(\$arguments[1], \$name, \$arguments[0], '\\{$parentClass->name}');
   }";
         # We will add every method that was declared as public in the given
         # class. Methods in reserved namespace (beginning with __) will not be
         # added, as well as inherited (but not overwritten) methods.
   ...
   private static function generateAutoboxingMethod(ReflectionFunctionAbstract $m,
      ...
      # allow programmers not to bother whether FIM will be able to parse their
      # docblock. However, we can only do that because of we're in generating
      # the cache (which will hopefully only occur once).
      $annotations = explode("\n",
         str_replace(["\r\n", "\r"], ["\n", "\n"],
      ...
      foreach($m->getParameters() as $parameter) {
         ...
         throw new \\BadMethodCallException('Missing argument \"$name\" for ' . \\Autoboxing::\$currentlyCalling . '" .
      ...
      ";
         ...
         throw new \\BadMethodCallException('Missing argument \"$key\" for ' . \\Autoboxing::\$currentlyCalling . '" .
                                 ($m instanceof \ReflectionMethod ? "::{$m->getShortName()}"
                                       : '') . "()');
      ";
                           }
