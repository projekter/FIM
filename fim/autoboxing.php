...
namespace fim {
   ...
   abstract class Autoboxing {
      ...
      private static function setupFunctionCache(\ReflectionFunction $function) {
         $classFile = $function->getFileName();
         $hash = hash('md5', $classFile);
         ...
         if($function->isClosure()) {
            ...
            if($startLine !== $endLine)
               ...
               $functionName = 'Closure' . hash('md5',
                     "$startLine-$endLine:{$function->getNumberOfRequiredParameters()}:{$function->getNumberOfParameters()}:{$function->getDocComment()}");
         }else
...
Cache;
      ...
      ' . self::generateAutoboxingMethod($function);
         $cacheContent = preg_replace('#   // < new content >#',
            "   // < new content >
...
/** This is an autogenerated cache file. Do not change this file, it might be
  * overwritten at any time without notification.
  * The original file is {$file->getFileName()}.
  * Date of generation: $now.
  */
      ...
      return \\fim\\Autoboxing::callMethod(\$arguments[1], \$name, \$arguments[0], '\\{$parentClass->name}');
            ...
            foreach($class->getMethods() as $method) {
               ...
               if(!$method->isPublic() || (isset($name[1]) && $name[0] === '_' && $name[1] === '_') || $method->getDeclaringClass()->name !== $class->name)
                  continue;
               $code = self::generateAutoboxingMethod($method);
               $cacheContent .= "
   ...
   public static function $name(array \$arguments" . ($method->isStatic() ? '' : ', $obj') . ") {
      ...
      ";
                  ...
                  case 'filestring':
                     ...
                     break;
                  case 'get':
                     $value = "$param\\Request::get('$name', null, false)";
                     break;
                  default:
                     if(preg_match('/(get|param|post|session|file|fileStream|fileString)\\s*+:\\s*+(.*+)/i',
                        ...
                        switch(strtolower($matches[1])) {
                           case 'get':
                              $value = "$param\\Request::get('$key', null, false)";
                              break;
                           case 'param':
                              if($parameter->isOptional())
                                 $value = $param . var_export($parameter->getDefaultValue(),
      ...
      ";
               ...
               $value = "(isset(\$arguments['$name']) || array_key_exists('$name', \$arguments)) ? \$arguments['$name'] : \\Request::get('$name', null, false)";
            $parseTypeHint = true;
            if(isset($types[$name])) {
