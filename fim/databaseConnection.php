...
final class DatabaseConnection {
   ...
   private final function compute() {
      ...
      if(!empty($data['dsn'])) {
         ...
         if(preg_match('#^[^:]+#', $data['dsn'], $matches) !== 1)
            throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.invalid',
               [$this->connectionFile]));
         ...
         switch($data['driver']) {
            ...
            case 'firebird':
               if(empty($data['database']))
                  throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.missing',
               ...
               if(!empty($data['host'])) {
                  ...
                  throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.wrong.firebird',
                     [$this->connectionFile]));
               $dsn .= $data['database'];
               $this->initStmt = 'SET SQL DIALECT 3';
               break;
            ...
            case 'sqlite':
               ...
               break;
            case 'odbc':
               throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.wrong.odbc',
                  [$this->connectionFile]));
            case 'oracle':
               if(empty($data['database']))
                  throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.missing',
               ...
               if(!empty($data['host'])) {
                  ...
                  throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.wrong.oracle',
                     [$this->connectionFile]));
               $dsn .= $data['database'];
               break;
            case 'sqlsrv':
            ...
            case 'informix':
               ...
               break;
            default:
               throw new DatabaseException(I18N::getInternalLanguage()->get('database.data.driver',
   ...
   public final function getConnection() {
      ...
      try {
         ...
         throw new DatabaseException(I18N::getInternalLanguage()->get('database.connection.failed',
            ...
            elseif(is_object($value))
               if($value instanceof Blob)
                  ...
                  try {
                     ...
                     throw new DatabaseException(I18N::getInternalLanguage()->get('database.bind.object'));
                  }
            else
               throw new DatabaseException(I18N::getInternalLanguage()->get('database.bind.unknown'));
         }
      }
      return $stmt->execute();
   }
   ...
   public final function select($table, $where = null, array $bind = null,
      ...
      if(!$this->execute($statement, $bind))
         throw new DatabaseException(I18N::getInternalLanguage()->get('database.sql.selectError'));
   ...
   public final function update($table, array $newValues, array $whereColumns) {
      ...
      if(!empty($whereColumns)) {
         ...
         if(is_int(key($whereColumns))) {
            ...
            elseif($diff < 0)
               throw new DatabaseException(I18N::getInternalLanguage()->get('database.sql.updateError'));
         }else
