...
class PHPMailer
{
     ...
     * @type string
     */
    public $From = 'noreply@framework.log';

    /**
     * The From name of the message.
     * @type string
     */
    public $FromName = 'Framework';
     ...
     * To generate iCal events, use EasyPeasyICS or iCalcreator
    ...
    public function __construct($exceptions = false)
    {
        ...
        static $from;
        if(!isset($from)) {
           if(preg_match('/^([^<]+) <([^>]+)>$/', $tmp = Config::get('mailFrom'), $matches) === 1)
              $from = [$matches[2], $matches[1]];
           else
              $from = [$tmp, $tmp];
        }
        $this->From = $from[0];
        ...
        if ($this->isError()) {
            ...
            try {
                ...
                if(file_put_contents($file, $body) === false)
                   throw new phpmailerException($this->lang('signing') . ' temp stream could not be written to.');
    ...
    protected function attachAll($disposition_type, $boundary)
    {
        ...
        foreach ($this->attachment as $attachment) {
            ...
            if ($attachment[6] == $disposition_type) {
                ...
                $inclhash = hash('md5', serialize($attachment));
    ...
    protected function encodeFile($path, $encoding = 'base64')
    {
        try {
            ...
            return $this->encodeString(file_get_contents($path), $encoding);
        } catch (Exception $exc) {
            $this->setError($exc->getMessage());
            return '';
        }
        ...
        if (isset($images[2])) {
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (preg_match('#^data:(image[^;,]*)(;base64)?,#', $url, $match)) {
                    ...
                    $cid = hash('md5', $url) . '@phpmailer.0'; // RFC2392 S 2
